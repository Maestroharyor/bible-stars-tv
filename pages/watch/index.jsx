...
const WatchIndex = () => {
    ...
    return(
        <DefaultLayout title="Watch">
            <div className='grid grid-cols-1 lg:grid-cols-12 py-16 px-5 lg:px-10 divide-y lg:divide-y-0 lg:divide-x'>
                <div className='col-span-2 overflow-x-scroll overflow-x-scroll lg:overflow-x-hidden py-3 lg:py-0'>
                    <div className='flex flex-row lg:flex-col gap-8 lg:sticky top-[100px] '>
                                {
                                sidebarLinks.map(link => (
                                    <Link key={link.name} href={link.link}>
                                    <a className={`inline-flex items-center gap-4 text-lg hover:text-brand-red ${router.pathname === link.link ? "text-brand-red": "text-gray-700"}`}>
                                        <span>{link.icon}</span>
                ...
                <div className='col-span-10'>
                    <h1 className='text-xl text-center font-bold lg:text-3xl text-brand-yellow mt-14 lg:mt-0 mb-14'>Watch Latest of Our Shows</h1>
